...
from pyspats.datasets import (
    load_wheatdata,
    generate_field_trial_data,
    load_example_spatial_data,
    create_toy_example,
)
...
class TestWheatData:
    ...
    def test_wheatdata_structure(self):
        ...
        assert (
            isinstance(data["geno"].dtype, pd.CategoricalDtype)
            or data["geno"].dtype == "object"
        )
        assert (
            isinstance(data["rep"].dtype, pd.CategoricalDtype)
            or data["rep"].dtype == "object"
        )
...
class TestFieldTrialGeneration:
    ...
    def test_custom_parameters(self):
        ...
        data = generate_field_trial_data(n_rows=8, n_cols=6, n_genotypes=15, seed=123)
    ...
    def test_variance_parameters(self):
        ...
        data_high_spatial = generate_field_trial_data(
            spatial_variance=200.0, genotype_variance=10.0, error_variance=5.0, seed=456
        )

        # Low spatial variance
        data_low_spatial = generate_field_trial_data(
            spatial_variance=10.0, genotype_variance=10.0, error_variance=5.0, seed=456
        )
