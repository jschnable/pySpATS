...
class TestSpATS:
    ...
    def test_custom_family(self):
        ...
        count_data = self.data.copy()
        count_data["response"] = np.round(count_data["response"]).astype(int)
        model_pois = SpATS(
            response="response",
            genotype="genotype",
            spatial=("col", "row"),
...
class TestSpATSIntegration:
    ...
    def test_wheat_data_analysis(self):
        ...
        data = generate_field_trial_data(n_rows=10, n_cols=8, n_genotypes=20, seed=42)
    ...
    def test_large_dataset_performance(self):
        ...
        data = generate_field_trial_data(n_rows=20, n_cols=15, n_genotypes=50, seed=456)
        ...
        model = SpATS(
            response="response", genotype="genotype", spatial=("col", "row"), data=data
        )
