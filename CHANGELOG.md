# 0.3.0 — R-validated rewrite (2026-09-12)

- Replace the 2025 basis and fitting engines with a complete tensor P-spline
  translation and exact SAP updates, including overlapping penalties.
- Validate against R-generated fixtures, the actual wheat trial and bundled
  sorghum data; add independent GLS, covariance and prediction checks.
- Correct variance/precision scaling, offsets, missingness, effective dimensions
  and random-genotype heritability; expose convergence and conditional SEs.
- Add immutable spatial settings, keyword-only fitting, process-based batch
  jobs, DataFrame exports, persistent prediction and accurate diagnostics.
- Use genotype Schur elimination and stable linear algebra; remove CHOLMOD and
  scikit-learn requirements. Plotting is optional.
- Replace synthetic wheat stand-in with attributed R data. Retire disconnected
  numerical paths, misleading tests, and generated bytecode from source control.
- Rewrite documentation and examples; document methodological proposals without
  implementing unapproved changes to the model's statistical assumptions.

- Release as 0.3.0; record matched 10,000-plot/2,000-genotype Python/R runs:
  3.55× faster with fixed genotypes and 10.57× with random genotypes (three-run
  medians, one BLAS thread), matching fitted values within 1.12e-10.

See docs/migration.md for breaking changes and docs/validation.md for evidence.

---

The entries below describe the historical implementation and are not current
validation claims.
